...
# --- CORRECCIÓN: TU APP SE LLAMA SALON ---
APP_NAME = 'salon'  

def write_file(filename, content):
    path = os.path.join(APP_NAME, filename)
    if not os.path.exists(APP_NAME):
        print(f"ERROR CRÍTICO: No encuentro la carpeta {APP_NAME}. Verifica el nombre.")
        return
    print(f"Reparando archivo: {path}...")
    with open(path, 'w', encoding='utf-8') as f:
        f.write(content)

print("--- INICIANDO REPARACIÓN EN CARPETA 'SALON' ---")
...
print("--- ARCHIVOS REPARADOS EN LA CARPETA 'SALON' ---")
