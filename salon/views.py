    ...
    if request.method == 'POST':
        try:
            ...
            if config and config.telegram_token:
                ...
                try: requests.post(f"https://api.telegram.org/bot{config.telegram_token}/sendMessage", data={"chat_id": config.telegram_chat_id, "text": msg, "parse_mode": "Markdown"}, timeout=3)
                except: pass
    ...
    if request.method == 'POST':
        ...
        if config and config.bold_secret_key:
            try:
                ...
                payload = {
                    "name": "Suscripción PASO", "description": f"Plan Mensual {peluqueria.nombre}",
...
def panel_negocio(request):
        ...
        if accion == 'test_telegram':
            ...
            try:
                r = requests.post(f"https://api.telegram.org/bot{token}/sendMessage", 
                                  data={"chat_id": chat, "text": "✅ *PASO Manager:* Conexión Exitosa.", "parse_mode": "Markdown"}, timeout=5)
    ...
    ctx = {
        'peluqueria': peluqueria, 'alerta_pago': alerta, 'proximo_pago': proximo,
        'citas_hoy_count': citas_hoy.count(), 'citas_futuras': citas_futuras,
...
def mi_agenda(request):
    ...
    lista = [{'id': i, 'nombre': n, 'trabaja': horarios.get(i) is not None, 
              'inicio': horarios.get(i).hora_inicio.strftime('%H:%M') if horarios.get(i) else '09:00',
              'fin': horarios.get(i).hora_fin.strftime('%H:%M') if horarios.get(i) else '19:00',
              'l_ini': horarios.get(i).almuerzo_inicio.strftime('%H:%M') if (horarios.get(i) and horarios.get(i).almuerzo_inicio) else '',
              'l_fin': horarios.get(i).almuerzo_fin.strftime('%H:%M') if (horarios.get(i) and horarios.get(i).almuerzo_fin) else ''} 
             for i, n in {0:'Lunes',1:'Martes',2:'Miércoles',3:'Jueves',4:'Viernes',5:'Sábado',6:'Domingo'}.items()]
...
def gestionar_ausencias(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            a.empleado = empleado; a.save()
            messages.success(request, "Ausencia guardada.")
...
def eliminar_ausencia(request, ausencia_id):
    get_object_or_404(Ausencia, id=ausencia_id, empleado__user=request.user).delete()
    return redirect('gestionar_ausencias')
...
# --- 6. PÚBLICO ---
def inicio(request):
    return render(request, 'salon/index.html', {'peluquerias': Peluqueria.objects.all(), 'ciudades': Peluqueria.objects.values_list('ciudad', flat=True).distinct()})
...
def agendar_cita(request, slug_peluqueria):
    ...
    if request.method == 'POST':
        try:
            ...
            def _reserva(emp):
                ...
                c = Cita.objects.create(peluqueria=peluqueria, empleado=emp, cliente_nombre=request.POST.get('nombre_cliente'), cliente_telefono=request.POST.get('telefono_cliente'), 
                                        fecha_hora_inicio=ini, fecha_hora_fin=ini + duracion, precio_total=precio, estado='P', metodo_pago=metodo)
                c.servicios.set(servs); return c
            ...
            if metodo == 'BOLD' and peluqueria.bold_secret_key:
                ...
                cita.enviar_notificacion_telegram() # Notificar reserva nequi
    ...
    return render(request, 'salon/agendar.html', {'peluqueria': peluqueria, 'servicios': peluqueria.servicios.all(), 'empleados': peluqueria.empleados.filter(activo=True), 
                                                  'tiene_bold': bool(peluqueria.bold_secret_key), 'tiene_nequi': bool(peluqueria.nequi_celular)})

def confirmacion_cita(request, slug_peluqueria, cita_id):
    """ Genera Link de Bold usando API y redirige """
    cita = get_object_or_404(Cita, id=cita_id)
    peluqueria = cita.peluqueria
    
    if not peluqueria.bold_secret_key:
        return render(request, 'salon/confirmacion.html', {'cita': cita, 'mensaje': 'Pago en sitio (Sin configuración Bold)'})
    ...
    try:
        ...
        payload = {
            "name": f"Reserva Cita #{cita.id}", 
            "description": f"Abono servicio peluquería",
            "amount": abono, "currency": "COP", "sku": ref,
            "redirection_url": request.build_absolute_uri(reverse('retorno_bold'))
        }
        
        r = requests.post(url, json=payload, headers=headers, timeout=8)
        if r.status_code == 201:
            cita.abono_pagado = abono
            cita.referencia_pago = ref
            cita.save()
            return redirect(r.json()["payload"]["url"]) # Redirige a Bold
        else:
            logger.error(f"Bold Error: {r.text}")
            return render(request, 'salon/confirmacion.html', {'cita': cita, 'mensaje': 'Error conectando con Bold. Paga en sitio.'})
            
    except Exception as e:
        logger.error(f"Bold Exception: {e}")
        return render(request, 'salon/confirmacion.html', {'cita': cita, 'mensaje': 'Error técnico. Paga en sitio.'})

def api_obtener_horarios(request):
    try:
        f = datetime.strptime(request.GET.get('fecha'), '%Y-%m-%d').date()
        e = Empleado.objects.get(id=request.GET.get('empleado_id'))
        return JsonResponse({'horas': obtener_bloques_disponibles(e, f, timedelta(minutes=30))})
