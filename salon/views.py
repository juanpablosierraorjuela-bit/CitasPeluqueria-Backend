...
# =======================================================
# 2. REGISTRO SAAS Y PAGO DE SUSCRIPCIÓN (TU LÓGICA DE COBRO)
# =======================================================
...
def landing_saas(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # NOTIFICACIÓN TELEGRAM AL SUPER ADMIN (TÚ)
            ...
            # LOGIN Y REDIRECCIÓN A PAGO (NO AL DASHBOARD)
            login(request, user)
...
def pago_suscripcion_saas(request):
    """
    Vista intermedia para cobrar la suscripción usando TUS credenciales maestras.
    """
    if not hasattr(request.user, 'perfil') or not request.user.perfil.es_dueño:
        return redirect('inicio')
    
    # Obtenemos TUS credenciales desde el admin
    config = ConfiguracionPlataforma.objects.first()
