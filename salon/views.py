        ...
        if form.is_valid():
            ...
            try:
                with transaction.atomic():
                    # 1. Crear Usuario (La señal crea el PerfilUsuario automáticamente)
...
# (gestionar_servicios y eliminar_servicio se mantienen igual, el código anterior funciona bien)
@login_required
...
def landing_saas(request):
    if request.method == 'POST':
        ...
        try:
            with transaction.atomic():
                # 1. Crear Usuario (Signal crea PerfilUsuario)
                ...
                # 3. ACTUALIZAR Perfil de Dueño (Corrección Critical Bug)
                perfil = user.perfil
                perfil.peluqueria = peluqueria
                ...
                Empleado.objects.create(
                    user=user, peluqueria=peluqueria, nombre="Administrador", apellido="(Dueño)", email_contacto=email, activo=True
                )
        ...
        try:
            ...
            def _crear_reserva_segura(empleado_bloqueado):
                conflicto = Cita.objects.filter(
                    empleado=empleado_bloqueado, estado__in=['P', 'C'],
                    fecha_hora_inicio__lt=fin_dt, fecha_hora_fin__gt=inicio_dt
                ).exists()
                
                if conflicto: raise ValueError("Horario ya ocupado.")

                cita = Cita.objects.create(
                    peluqueria=peluqueria, empleado=empleado_bloqueado,
                    cliente_nombre=cliente_nombre, cliente_telefono=cliente_telefono,
                    fecha_hora_inicio=inicio_dt, fecha_hora_fin=fin_dt,
                    precio_total=precio_final, descuento_aplicado=descuento, abono_pagado=abono,
                    estado='P' # PENDIENTE DE PAGO (Para ir a confirmar)
                )
...
def confirmacion_cita(request, slug_peluqueria, cita_id):
    ...
    # Si la cita ya está confirmada o es pendiente, mostramos la info
    # Aquí podríamos agregar la lógica de firma de Bold si estado es 'P'
