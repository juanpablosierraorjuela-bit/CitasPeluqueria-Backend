...
# Contenido COMPLETO y CORREGIDO de salon/views.py
views_code = """import logging, json, requests, math
...
def panel_negocio(request):
    ...
    if request.method == 'POST':
        ...
        elif accion == 'test_telegram':
            token = request.POST.get('telegram_token')
            chat = request.POST.get('telegram_chat_id')
            peluqueria.telegram_token = token
            peluqueria.telegram_chat_id = chat
            peluqueria.save()
            try:
                requests.post(f"https://api.telegram.org/bot{token}/sendMessage", data={"chat_id": chat, "text": "✅ *Conexión Exitosa*", "parse_mode": "Markdown"})
                messages.success(request, "Telegram conectado.")
            except:
                messages.error(request, "Error conectando Telegram.")
...
def agendar_cita(request, slug_peluqueria):
    ...
    if request.method == 'POST':
        try:
            if not request.POST.get('acepta_politicas'): raise ValueError("Debes aceptar las políticas de cancelación.")
            ...
            if es_domicilio and not peluqueria.ofrece_domicilio: raise ValueError("Este establecimiento no ofrece servicio a domicilio.")
            metodo = request.POST.get('metodo_pago', 'SITIO')
            if solo_pago_digital and metodo == 'SITIO': raise ValueError("Debes seleccionar un método de pago digital.")
            ...
            def _reserva(emp):
                ...
                c = Cita.objects.create(peluqueria=peluqueria, empleado=emp, cliente_nombre=request.POST.get('nombre_cliente'), cliente_telefono=request.POST.get('telefono_cliente'), fecha_hora_inicio=ini, fecha_hora_fin=ini + duracion, precio_total=precio, estado='P', metodo_pago=metodo, tipo_cobro=tipo_cobro, es_domicilio=es_domicilio, direccion_domicilio=request.POST.get('direccion_domicilio') if es_domicilio else None)
                c.servicios.set(servs)
                return c
            
            cita = BookingManager.ejecutar_reserva_segura(emp_id, _reserva)
            
            if metodo == 'BOLD' and tiene_bold: return redirect('procesar_pago_bold', cita_id=cita.id)
            elif metodo == 'NEQUI' and tiene_nequi: cita.enviar_notificacion_telegram(); return render(request, 'salon/pago_nequi.html', {'cita': cita, 'peluqueria': peluqueria})
            else:
                cita.estado = 'C'; cita.save()
                for s in cita.servicios.all():
                    if s.producto_asociado:
                        p = s.producto_asociado; p.cantidad_actual -= s.cantidad_descuento; p.save()
            ...
            return render(request, 'salon/agendar.html', {'peluqueria': peluqueria, 'servicios': peluqueria.servicios.all(), 'empleados': peluqueria.empleados.filter(activo=True), 'tiene_bold': tiene_bold, 'tiene_nequi': tiene_nequi, 'solo_pago_digital': solo_pago_digital, 'error_mensaje': str(e)})
            
    return render(request, 'salon/agendar.html', {'peluqueria': peluqueria, 'servicios': peluqueria.servicios.all(), 'empleados': peluqueria.empleados.filter(activo=True), 'tiene_bold': tiene_bold, 'tiene_nequi': tiene_nequi, 'solo_pago_digital': solo_pago_digital})
...
def procesar_pago_bold(request, cita_id):
    ...
    if not peluqueria.bold_secret_key: return render(request, 'salon/confirmacion.html', {'cita': cita, 'mensaje': 'Error: Comercio sin llave secreta configurada.'})
    try:
        ...
        r = requests.post(url, json=payload, headers=headers, timeout=10)
        if r.status_code == 201: cita.referencia_pago = ref; cita.save(); return redirect(r.json()["payload"]["url"])
        else: return render(request, 'salon/confirmacion.html', {'cita': cita, 'mensaje': 'No se pudo generar el link de pago con Bold.'})
    except Exception as e: return render(request, 'salon/confirmacion.html', {'cita': cita, 'mensaje': 'Error de conexión con la pasarela.'})
...
def retorno_bold(request):
    ...
    else: return render(request, 'salon/confirmacion.html', {'cita': cita, 'mensaje': 'El pago no fue aprobado. Intenta nuevamente.', 'error': True})
...
print("✅ views.py reparado exitosamente.")
