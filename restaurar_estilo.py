...
<body>
    ...
    <nav class="navbar" id="navbar">
        <a href="https://pasotunja.com" class="brand-pill"><div class="dot"></div> PASO</a>
    ...
    <header>
        ...
        <p class="hero-subtitle">Reserva en los mejores salones y barberías.<br>Sin llamadas, sin esperas.</p>
    ...
    <div class="grid-wrapper" id="salon-grid">
        {% for p in peluquerias %}
            <a href="{% url 'agendar_cita' p.slug %}" class="salon-card" data-ciudad="{{ p.ciudad|default:'General' }}">
                <div class="card-header">
                    <div class="salon-avatar">{{ p.name|first|upper }}</div>
                    <div class="salon-info"><h3>{{ p.name }}</h3><span>{{ p.ciudad|default:"Tunja" }}</span></div>
                </div>
                <p class="address">{{ p.address|default:"Ubicación: Centro Comercial" }}</p>
                
                <div class="social-row" onclick="event.stopPropagation();"> 
                    {% if p.instagram %} <object><a href="{{ p.instagram }}" target="_blank" class="social-icon instagram"><i class="fab fa-instagram"></i></a></object> {% endif %}
                    {% if p.facebook %} <object><a href="{{ p.facebook }}" target="_blank" class="social-icon facebook"><i class="fab fa-facebook-f"></i></a></object> {% endif %}
                </div>
        ...
        {% empty %}
            <div style="grid-column: 1/-1; text-align: center; padding: 60px; color: #94a3b8;">
                ...
                <p>No hay salones registrados aún. ¡Registra el tuyo!</p>
            </div>
        {% endfor %}
    </div>
    <script>
